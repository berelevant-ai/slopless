...
require_contains README.md "https://github.com/berelevant-ai/slopless/wiki/Philosophy"
require_contains README.md "Developed by [berelevant.ai](https://berelevant.ai) to keep content specific, useful, and recognizably human."
...
if (pkg.repository?.url !== "git+https://github.com/berelevant-ai/slopless.git") {
  throw new Error(`bad repository.url: ${pkg.repository?.url}`);
}
if (pkg.homepage !== "https://github.com/berelevant-ai/slopless#readme") {
  throw new Error(`bad homepage: ${pkg.homepage}`);
}
if (pkg.bugs?.url !== "https://github.com/berelevant-ai/slopless/issues") {
  throw new Error(`bad bugs.url: ${pkg.bugs?.url}`);
}
'

require_contains .github/ISSUE_TEMPLATE/config.yml "https://github.com/berelevant-ai/slopless/discussions"
require_not_contains .github/ISSUE_TEMPLATE/config.yml "https://github.com/agent-quality-controls/slopless/discussions"
require_contains .github/CONTRIBUTING.md "git clone https://github.com/berelevant-ai/slopless.git"
require_not_contains .github/CONTRIBUTING.md "git clone https://github.com/agent-quality-controls/slopless.git"

remote_url="$(git remote get-url origin)"
if [[ "$remote_url" != "https://github.com/berelevant-ai/slopless.git" ]]; then
  fail "origin remote is $remote_url"
fi

repo_json="$(gh repo view berelevant-ai/slopless --json nameWithOwner,defaultBranchRef,description,homepageUrl,hasIssuesEnabled,hasWikiEnabled,hasDiscussionsEnabled,repositoryTopics)"
node -e '
...
assert(repo.nameWithOwner === "berelevant-ai/slopless", `bad repo ${repo.nameWithOwner}`);
assert(repo.defaultBranchRef.name === "main", `bad default branch ${repo.defaultBranchRef.name}`);
assert(repo.description === "Deterministic textlint rules and CLI for catching prose slop in English Markdown.", `bad description ${repo.description}`);
...
actions_json="$(gh api repos/berelevant-ai/slopless/actions/permissions)"
node -e '
const actions = JSON.parse(process.argv[1]);
if (actions.enabled !== true) throw new Error("actions disabled");
' "$actions_json"

gh api repos/berelevant-ai/slopless/vulnerability-alerts >/dev/null
gh api repos/berelevant-ai/slopless/branches/main/protection >/dev/null
gh repo view berelevant-ai/.github --json nameWithOwner,description,defaultBranchRef >/dev/null

printf 'PASS berelevant-ai migration\n'
